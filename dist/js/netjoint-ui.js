...
define('datepicker', ['jquery'], function () {
    !function ($, undefined) {
        ...
        Datepicker.prototype = {
            ...
            show: function (e) {
                if (e && e.type === 'focus' && this.picker.is(':visible'))
                    return;
                if (!this.isInline)
                ...
                if (arguments.length) {
                    $.each(arguments, $.proxy(function (i, date) {
                        if (this.o.timepicker && i === 0) {
                            this.timepicker.update(this._getTime(date));
                        }
                    ...
                    if (dates && this.o.timepicker) {
                        ...
                        if (tokens.length === 2) {
                            dates = tokens[0];
                            this.timepicker.update(tokens[1], true);
                        }
                    }
            ...
            fillDow: function () {
                var dowCnt = this.o.weekStart, html = '<tr class="week-content">';
                if (this.o.calendarWeeks) {
    ...
    !function ($) {
        ...
        TimePicker.prototype = {
            ...
            _place: function () {
                if (this.isInline || this.isInDatepicker)
                    return;
                var calendarWidth = this.picker.outerWidth(), calendarHeight = this.picker.outerHeight(), visualPadding = 10, $window = $(window), windowWidth = $window.width(), windowHeight = $window.height(), scrollTop = $window.scrollTop();
                var zIndex = parseInt(this.element.parents().filter(function () {
                    return $(this).css('z-index') !== 'auto';
                }).first().css('z-index')) + 10;
                var offset = this.component ? this.component.parent().offset() : this.element.offset();
            ...
            _pickerEvents: function () {
                ...
                this.picker.on('click', '.J_up', function (ev) {
                    var target = ev.currentTarget, parentNode = $(target).parent(), role = parentNode.attr('data-role');
                    self._slide(role, 'up');
                }).on('click', '.J_down', function (ev) {
                    var target = ev.currentTarget, parentNode = $(target).parent(), role = parentNode.attr('data-role');
                    self._slide(role, 'down');
                }).on('click', 'span', function (ev) {
                    var target = ev.currentTarget, parentNode = $(target).parent().parent().parent(), role = parentNode.attr('data-role'), targetNum = target.innerHTML, attrs = self[role + 'Attr'], step = parseInt(targetNum - attrs.current, 10), dur;
                    if (step > 0) {
            ...
            _slideDonw: function (attrs, step, notSetValue) {
                step = step || 1;
                var cp = attrs.cp, dur = attrs.ih * step;
                attrs.current += step;
            ...
            _updateUI: function () {
                var oldMimute = this.o.minute, oldHour = this.o.hour, attrs, role, step;
                this._getInputTime();
            ...
            _juicer: function (current, list) {
                ...
                return '<div class="picker-wrap">' + '<a href="javascript:;" class="picker-btn up J_up"><b class="arrow"></b><b class="arrow-bg"></b></a>' + '<div class="picker-con">' + '<div class="picker-innercon">' + items + '</div>' + '</div>' + '<a href="javascript:;" class="picker-btn down J_down"><b class="arrow"></b><b class="arrow-bg"></b></a>' + '</div>';
            },
            _renderHour: function () {
                var self = this, hourRet = [];
                for (var i = 0; i < 24; i++) {
                    hourRet.push(self._beautifyNum(i));
                }
                var tpl = this._juicer(self.o.hour, hourRet), $tpl = $(tpl);
            ...
            _renderMinutes: function () {
                var self = this, minuteRet = [];
                for (var i = 0; i < 60; i++) {
                    minuteRet.push(self._beautifyNum(i));
                }
                var tpl = this._juicer(self.o.minute, minuteRet), $tpl = $(tpl);
            ...
            _addPrefixAndSuffix: function (parentNode, maxSize) {
                var self = this, pickerCon = $('.picker-con', parentNode), innerPickerCon = $('.picker-innercon', parentNode), currentNode = $('.current', parentNode), itemH = currentNode.outerHeight(), parentH = pickerCon.outerHeight(), fixNum = Math.floor(parentH / itemH) + 1, currentNodeOffsetTop, currentPosition, tpl = '';
            ...
            _renderSplit: function () {
                var tpl = '<div class="timePicker-split">' + '<div class="hour-input"></div>' + '<div class="split-icon">:</div>' + '<div class="minute-input"></div>' + '</div>';
                this.picker.append($(tpl));
            },
            ...
            _setValue: function () {
                if (this.isInline)
                    return;
                this._doSetValue(this._getCurrentTimeStr());
            },
            _animate: function (node, dur) {
                if ($.support.transition) {
                    node.css({ 'top': dur + 'px' });
                } else {
                    node.animate({ top: dur + 'px' }, 300);
                }
            },
        ...
        $.fn.timepicker = function (option) {
            ...
            var internal_return;
            this.each(function () {
                var $this = $(this), data = $this.data('timepicker');
                if (!data)
                    $this.data('timepicker', data = new TimePicker(this, option));
                if (typeof option === 'string' && typeof data[option] === 'function') {
                    internal_return = data[option].apply(data, args);
                    if (internal_return !== undefined)
                        return false;
        ...
        $(document).on('focus.timepicker.data-api click.timepicker.data-api', '[data-toggle="timepicker"]', function (e) {
            var $this = $(this);
...
define('editable', [
    ...
    (function ($) {
        ...
        Select.defaults = $.extend({}, $.fn.editabletypes.list.defaults, { tpl: '<select></select>' });
    ...
    (function ($) {
        ...
        Password.defaults = $.extend({}, $.fn.editabletypes.text.defaults, { tpl: '<input type="password">' });
    ...
    (function ($) {
        ...
        Email.defaults = $.extend({}, $.fn.editabletypes.text.defaults, { tpl: '<input type="email">' });
    ...
    (function ($) {
        ...
        Url.defaults = $.extend({}, $.fn.editabletypes.text.defaults, { tpl: '<input type="url">' });
    ...
    (function ($) {
        ...
        Tel.defaults = $.extend({}, $.fn.editabletypes.text.defaults, { tpl: '<input type="tel">' });
    ...
    (function ($) {
        ...
        Time.defaults = $.extend({}, $.fn.editabletypes.abstractinput.defaults, { tpl: '<input type="time">' });
        ...
        Combodate.prototype = {
            ...
            fillYear: function () {
                ...
                for (i = this.options.maxYear; i >= this.options.minYear; i--) {
                    ...
                    items[this.options.yearDescending ? 'push' : 'unshift']([
                        i,
                        name
                    ]);
                }
    ...
    (function ($) {
        ...
        $.extend(Date.prototype, {
            initPicker: function (options, defaults) {
                ...
                this.options.datepicker = $.extend({}, defaults.datepicker, options.datepicker, { format: this.options.viewformat });
    ...
    (function ($) {
        ...
        $.extend(DateTime.prototype, {
            initPicker: function (options, defaults) {
                ...
                this.options.datetimepicker = $.extend({}, defaults.datetimepicker, options.datetimepicker, { format: this.options.viewformat });
